  ...
  return (
    <RoleLayout menuItems={KAUR_TEKNIS_MENU} userName="Budi Santoso" userRole="Kaur Teknis">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 max-w-4xl mb-8">
        <form className="flex flex-col gap-8" onSubmit={e => e.preventDefault()}>
          ...
          <div className="pt-6 border-t border-slate-200 flex justify-end">
            <button
              type="button"
              disabled={!dusun || !judul || !kategori || !volume || !pagu || !formulirMusrembangFile || !rabFile}
              onClick={() => setShowModal(true)}
              className={`px-8 py-4 text-white font-bold rounded-xl shadow-lg transition-all uppercase tracking-wide text-sm flex items-center gap-2 ${
                (!dusun || !judul || !kategori || !volume || !pagu || !formulirMusrembangFile || !rabFile) 
      ...
      <PinModal
        ...
        description="Masukkan 6 digit PIN kredensial Anda untuk menyetujui dan mengunci usulan ini."
        onConfirm={async (pin) => {
          setShowModal(false);
          try {
