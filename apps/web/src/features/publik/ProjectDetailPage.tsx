              ...
              {project.photos && project.photos.length > 0 ? project.photos.map((photo: any) => (
                <div key={photo.id} className="relative group rounded-xl overflow-hidden border border-slate-200 bg-slate-100">
                  <div className="aspect-[4/3] w-full">
                    <img src={getMediaUrl(photo.url)} alt="Progress" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <div className="mb-6 bg-white border border-slate-200 rounded-xl p-4">
              ...
              <div className="flex flex-col gap-2">
                {project.formulirMusrembangUrl ? (
                  <a href={getMediaUrl(project.formulirMusrembangUrl)} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between text-xs p-2 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200">
                    <span className="font-medium text-slate-700">Formulir Musrenbang</span>
                   ...
                   <div className="text-xs text-slate-400 p-2 italic bg-slate-50 rounded-lg border border-slate-100">Formulir Musrenbang belum diunggah</div>
                )}
                {project.rabUrl ? (
                  <a href={getMediaUrl(project.rabUrl)} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between text-xs p-2 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200">
                    <span className="font-medium text-slate-700">Rincian Anggaran (RAB)</span>
                    <span className="text-blue-600 font-bold hover:underline flex items-center gap-1">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                      Unduh PDF
                    </span>
                   ...
                   <div className="text-xs text-slate-400 p-2 italic bg-slate-50 rounded-lg border border-slate-100">RAB belum diunggah</div>
                )}
                {project.lpjKeuanganUrl && (
                  <a href={getMediaUrl(project.lpjKeuanganUrl)} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between text-xs p-2 bg-green-50 hover:bg-green-100 rounded-lg transition-colors border border-green-200 mt-2">
                    <div>
                      <span className="font-bold text-green-800 block mb-0.5">LPJ Keuangan Final</span>
                      <span className="font-mono text-[10px] text-green-600 block truncate w-40">{project.lpjKeuanganHash?.substring(0,20)}...</span>
                    </div>
                    <span className="text-green-700 font-bold hover:underline flex flex-col items-end gap-1">
                      <span className="flex items-center gap-1">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 002-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                        Terkunci On-Chain
                      </span>
                      Unduh PDF
                    </span>
                ...
                <div key={t.id} className="bg-white border border-slate-200 p-4 rounded-xl">
                  ...
                  {t.beritaAcaraUrl && (
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                      <span className="text-slate-500">Berita Acara Pencairan:</span>
                      <a href={getMediaUrl(t.beritaAcaraUrl)} target="_blank" rel="noopener noreferrer" className="text-blue-600 font-bold hover:underline flex items-center gap-1">
                  ...
                  {t.lpjTeknisUrl && (
                    <div className={`mt-2 ${!t.beritaAcaraUrl ? 'pt-2 border-t border-slate-100' : ''} flex items-center justify-between text-xs`}>
                      <span className="text-slate-500">Dokumen SPJ/LPJ:</span>
                      <a href={t.lpjTeknisUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 font-bold hover:underline flex items-center gap-1">
