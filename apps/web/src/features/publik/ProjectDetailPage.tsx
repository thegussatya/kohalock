              ...
              {project.photos && project.photos.length > 0 ? project.photos.map((photo: any) => (
                <div key={photo.id} className="relative group rounded-xl overflow-hidden border border-slate-200 bg-slate-100">
                  <div className="aspect-[4/3] w-full">
                    <img src={photo.url?.startsWith('http') ? photo.url : `http://localhost:3000${photo.url}`} alt="Progress" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
            ...
            <div className="mb-6 bg-white border border-slate-200 rounded-xl p-4">
              ...
              <div className="flex flex-col gap-2">
                {project.formulirMusrembangUrl ? (
                  <a href={`http://localhost:3000${project.formulirMusrembangUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between text-xs p-2 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200">
                ...
                {project.rabUrl ? (
                  <a href={`http://localhost:3000${project.rabUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between text-xs p-2 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200">
                ...
                {project.lpjKeuanganUrl && (
                  <a href={project.lpjKeuanganUrl.startsWith('http') ? project.lpjKeuanganUrl : `http://localhost:3000${project.lpjKeuanganUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between text-xs p-2 bg-green-50 hover:bg-green-100 rounded-lg transition-colors border border-green-200 mt-2">
                    <div>
                ...
                <div key={t.id} className="bg-white border border-slate-200 p-4 rounded-xl">
                  ...
                  {t.beritaAcaraUrl && (
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                      ...
                      <a href={`http://localhost:3000${t.beritaAcaraUrl}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 font-bold hover:underline flex items-center gap-1">
