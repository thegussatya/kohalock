...
roles.forEach(role => {
  const content = `import SharedProfilePage from '../shared/ProfilePage';
...
`;
  const filePath = path.join(process.cwd(), 'src/features', role.name, 'ProfilePage.tsx');
  fs.writeFileSync(filePath, content, 'utf8');
  console.log('Created ' + filePath);

  routerImports += `import ${role.varName}Profile from '../features/${role.name}/ProfilePage';\n`;
  routerRoutes += `  {\n    path: '/${role.name}/profil',\n    element: <${role.varName}Profile />,\n  },\n`;
});
...
// Inject imports at line 37 (after the last BpdAdatNotifications import)
const lines = routerContent.split('\\n');
const insertIndex = lines.findIndex(l => l.includes('import BpdAdatNotifications')) + 1;
lines.splice(insertIndex, 0, routerImports.trimEnd());
